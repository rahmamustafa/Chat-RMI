...
import gov.iti.io.Client.RMIClient;
...
public class chatController implements Initializable {
    ...
    void sendMessage() {
        if (!tfSendMessage.getText().isBlank()) {
            ...
            client.tell(tfSendMessage.getText());
            tfSendMessage.clear();
    ...
    @FXML
    public void acceptGroupMessage(String name,String friendImageUrl,String message) {
        System.out.println(name+"message:"+message);
        friendName=name;
        if (!message.isBlank()) {
            vbMessagesContainer.getChildren()
                    .add(createSentMesseage(message,friendImageUrl, "left"));
            scrollPane.setVvalue(scrollPane.getVmax());
        }
    ...
    public void initialize(URL arg0, ResourceBundle arg1) {
        ...
        client = new RMIClient(this ,user.getName(),user.getImageUrl());
    }

    public HBox createSentMesseage(String messageText,String friendImage, String alignment) {
        ...
        if (alignment == "right") {
            ...
            Image image = new Image(friendImage);
            imageContainer.setFill(new ImagePattern(image));
    ...
    public void setFriendImage(String friendImageUrl){
        this.friendImageUrl = user.getImageUrl();
    }
    public void welcomeMessage(String welcomeMessage){
        System.out.println("run welcome message: " + welcomeMessage);
